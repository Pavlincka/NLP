...
async def get_page_urls(session, start_date, finish_date): # год - месяц - день
...
async def main():
    ...
    async with aiohttp.ClientSession() as session:
        page_urls = await get_page_urls(session, [2018, 1, 1], [2022, 12, 31])
